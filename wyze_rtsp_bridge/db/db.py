...
class WyzeRtspDatabase:
    def __init__(self, config: Config, echo=False):
        self.config = config
    ...
    def open(self):
        ...
        self.engine = sqlalchemy.create_engine(f"sqlite:///{path}", echo=self.echo, future=True)
    ...
    with db.session() as session:
        result = session.execute(
            select(models.Credential)
                .where(models.Credential.id == models.CREDENTIAL_ID)
        ).first()

    if result is None:
        return None

    from_orm: models.CredentialModel = models.CredentialModel.from_orm(result[0])
    return from_orm


def set_credentials(db: WyzeRtspDatabase, auth_info: wyzecam.api_models.WyzeCredential) -> None:
    with db.session() as session:
